...
function Button({ text, logo, path, style }: ButtonProps) {
    return (
        <TouchableOpacity onPress={() => { (router.push as any)({ pathname: path }) }} style={[{width: '30%', aspectRatio: 1 / 1, backgroundColor: globalColorTheme.backgroundColor, borderRadius: 10, justifyContent: 'center', alignItems: 'center',}, style]}>
    ...
    return (
        <View style={{backgroundColor: globalColorTheme.backgroundColor, width: Dimensions.get('screen').width, height: Dimensions.get('screen').height * 0.22}}>
            ...
            <View style={{width: '100%', height: '85%', display: 'flex', flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center', position: 'relative', top: '-2.5%',}}>
                <Button text='Resident' logo='home' path="/(tabs)/ResidentTab" style={{shadowColor: globalColorTheme.color, shadowOffset: {width: -2, height: 4}, shadowOpacity: 0.33, shadowRadius: 10, elevation: 5}}/>
                <Button text='Visitor' logo='location-arrow' path="/(tabs)/VisitorTab" style={{shadowColor: globalColorTheme.color, shadowOffset: {width: -2, height: 4}, shadowOpacity: 0.33, shadowRadius: 10, elevation: 5}}/>
                <Button text='Business' logo='suitcase'  path="/(tabs)/BusinessTab" style={{shadowColor: globalColorTheme.color, shadowOffset: {width: -2, height: 4}, shadowOpacity: 0.33, shadowRadius: 10, elevation: 5}}/>
            </View>
        </View>
    )
}
